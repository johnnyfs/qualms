...
emitter). It is descriptive of what the engine accepts today, not aspirational.
Anything documented in `stories/tutorial/tutorial.qualms` but not parsed or
executed by the current implementation is listed in `README.md` under TODO.

Status: normative for branch `js-migration` at the point this spec was
authored. Features being actively added by other agents may outpace it; keep
the README TODO list current as items land.
...
parameter type expressions. A parameter prefixed with `one` is a cardinality
constraint (§ 4.3). The parameter type may name a trait, another relation
symbol, or `Any`.
...
### 4.3 Cardinality (`one`) on relations

`relation R(A, one B)` declares that, in the projection of `R` over `A`,
each `A` may be associated with at most one `B`. Asserting `R(a, b1)`
retracts any pre-existing `R(a, *)` fact (§ 6.3).
...
2. **Bind further names** — a constraint that succeeds may introduce
   additional bindings for free identifiers that appear in it. Those
...
- **Relation atom.** If the head identifier names a declared predicate,
  ...
  environment, otherwise zero. If the head names a relation symbol with
  stored facts, the runtime walks the fact base and produces one
  environment per matching fact, binding any free identifiers in the
  pattern.
...
- **Equality (`==`).** Both operands are resolved against the environment.
  ...
  free identifier, the runtime unifies the identifier with the other
  operand and returns the augmented environment.

#### 5.5.1 Term shapes

Terms used in expressions include identifiers (free or bound), numbers,
strings, the wildcard `_`, and relation instances `R(t1, …, tn)`.
...
A free identifier referenced in a relation pattern is bound by the match
if the corresponding fact argument is concrete. Subsequent uses of the
identifier within the same conjunction are constrained to that same
value.

Identifiers that appear in an expression and *also* name a declared
entity are pre-bound when the MCP `query` tool builds its base
environment (see `protocol.md` § 3.2). Inside a callable, parameter names
are pre-bound to the supplied arguments.
...
A relation parameter declared `one T` is the *primary* axis of the
...
`reasons` is the deduplicated list of individual reasons. Pass results
always have empty reasons today.
